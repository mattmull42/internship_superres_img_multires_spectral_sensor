...
def run_pipeline_v2_tests(input_names, niter):
    ...
    with open(path.join('output', 'errors_log.csv'), 'a') as errors_log:
        ...
        for input_name in tqdm(input_names, desc='Processed inputs ', colour='green'):
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Pipeline V2', input_name[:-4] + '_bayer', f'{duration:.2f}', np.mean(mse_bayer), np.mean(ssim_bayer)] + mse_bayer + ssim_bayer)
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Pipeline V2', input_name[:-4] + '_quad_bayer', f'{duration:.2f}', np.mean(mse_quad), np.mean(ssim_quad)] + mse_quad + ssim_quad)
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Pipeline V2', input_name[:-4] + '_quad_bayer_binning', f'{duration:.2f}', np.mean(mse_binning), np.mean(ssim_binning)] + mse_binning + ssim_binning)
