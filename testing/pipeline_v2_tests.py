...
def run_pipeline_v2_tests(input_names, noise_level, niter, sigma, epsilon, box_constraint):
    ...
    with open(path.join('output', 'errors_log.csv'), 'a') as errors_log:
        ...
        for input_name in input_names:
            start = perf_counter()
            pipeline_bayer.run(input_name, noise_level)
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline v2 noise:{noise_level} niter:{niter}', input_name[:-4] + '_bayer', f'{duration:.2f}', f'{np.mean(mse_bayer):.4f}', f'{np.mean(ssim_bayer):.4f}'] + mse_bayer + ssim_bayer)
            
            start = perf_counter()
            pipeline_quad.run(input_name, noise_level)
            duration = perf_counter() - start
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline v2 noise:{noise_level} niter:{niter}', input_name[:-4] + '_quad_bayer', f'{duration:.2f}', f'{np.mean(mse_quad):.4f}', f'{np.mean(ssim_quad):.4f}'] + mse_quad + ssim_quad)

            start = perf_counter()
            pipeline_binning.run(input_name, noise_level)
            duration = perf_counter() - start
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline v2 noise:{noise_level} niter:{niter}', input_name[:-4] + '_quad_bayer_binning', f'{duration:.2f}', f'{np.mean(mse_binning):.4f}', f'{np.mean(ssim_binning):.4f}'] + mse_binning + ssim_binning)
