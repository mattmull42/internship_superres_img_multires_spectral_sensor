...
def run_pipeline_v2_tests(input_names, niter, sigma, epsilon):
    print(Fore.YELLOW + '###################### Beginning of the pipeline V2 tests ######################')

    create_output_dirs()

    pipeline_bayer = Pipeline_v2('bayer', False, niter, sigma, epsilon)
    pipeline_quad = Pipeline_v2('quad_bayer', False, niter, sigma, epsilon)
    pipeline_binning = Pipeline_v2('quad_bayer', True, niter, sigma, epsilon)
    ...
    with open(path.join('output', 'errors_log.csv'), 'a') as errors_log:
        ...
        for input_name in input_names:
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline V2 niter:{niter}', input_name[:-4] + '_bayer', f'{duration:.2f}', np.mean(mse_bayer), np.mean(ssim_bayer)] + mse_bayer + ssim_bayer)
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline V2 niter:{niter}', input_name[:-4] + '_quad_bayer', f'{duration:.2f}', np.mean(mse_quad), np.mean(ssim_quad)] + mse_quad + ssim_quad)
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline V2 niter:{niter}', input_name[:-4] + '_quad_bayer_binning', f'{duration:.2f}', np.mean(mse_binning), np.mean(ssim_binning)] + mse_binning + ssim_binning)
