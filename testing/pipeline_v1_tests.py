...
def run_pipeline_v1_tests(input_names, noise_level):
    ...
    with open(path.join('output', 'errors_log.csv'), 'a') as errors_log:
        ...
        for input_name in input_names:
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline v1 noise:{noise_level}', input_name[:-4] + '_bayer', f'{duration:.2f}', f'{np.mean(mse_bayer):.4f}', f'{np.mean(ssim_bayer):.4f}'] + mse_bayer + ssim_bayer)
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline v1 noise:{noise_level}', input_name[:-4] + '_quad_bayer', f'{duration:.2f}', f'{np.mean(mse_quad):.4f}', f'{np.mean(ssim_quad):.4f}'] + mse_quad + ssim_quad)
            ...
            csvwriter.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f'Pipeline v1 noise:{noise_level}', input_name[:-4] + '_quad_bayer_binning', f'{duration:.2f}', f'{np.mean(ssim_binning):.4f}', f'{np.mean(ssim_binning):.4f}'] + mse_binning + ssim_binning)
