...
def ISTA(niter, f, g, x_0, L_0=1, eta=1.1, precision=1e-6, verbose=False):
    if verbose:
        start = perf_counter()
    ...
    for k in range(1, niter + 1):
        ...
        while criteria:
            f_grad = f.grad(x_k)
            prox_step = g.prox(x_k - f_grad / L_k, L_k)
            g_prox_step = g(prox_step)
            diff = (prox_step - x_k).reshape(-1)

            if f(prox_step) + g_prox_step <= f(x_k) + np.dot(diff, f_grad.reshape(-1) + L_k / 2 * diff) + g_prox_step:
                criteria = False
